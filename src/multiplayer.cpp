...
// some stuff for getting real terrain elevation below the multiplayer pilot
static XPLMProbeRef mTerrainProbe = NULL;
 ...
 */
	...
	// getting the Clamping pref
	if ((section == "PREFERENCES" || section == "preferences") && (key == "CLAMPING" || key == "clamping")) {
		return xivap.Clamping();
	}
	//
...
_initialized(false), _enabled(false)
{
	...
	// terrain probe
	XPLMDestroyProbe(mTerrainProbe);
	mTerrainProbe = XPLMCreateProbe(xplm_ProbeHitTerrain);

	//debug plane stuff
	mDPlaneGearDeploy = XPLMFindDataRef("sim/cockpit/switches/gear_handle_status");

	mDPlaneBeaconRef = XPLMFindDataRef("sim/cockpit/electrical/beacon_lights_on");
	mDPlaneLandingRef = XPLMFindDataRef("sim/cockpit/electrical/landing_lights_on");
	mDPlaneNavRef = XPLMFindDataRef("sim/cockpit/electrical/nav_lights_on");
	mDPlaneStrobeRef = XPLMFindDataRef("sim/cockpit/electrical/strobe_lights_on");
	mDPlaneTaxiRef = XPLMFindDataRef("sim/cockpit/electrical/taxi_light_on");

	mDPlaneFlapsDeploy = XPLMFindDataRef("sim/flightmodel/controls/flaprat");
	mDPlaneSpeedBrakes = XPLMFindDataRef("sim/flightmodel/controls/sbrkrqst");	

	mDPLaneNumEngines = XPLMFindDataRef("sim/aircraft/engine/acf_num_engines");
	mDPlaneEngineMode = XPLMFindDataRef("sim/flightmodel/engine/ENGN_propmode");	
	mDPlaneEngineThrust = XPLMFindDataRef("sim/flightmodel/engine/ENGN_thro");
	mDPlaneIsEngineRunning = XPLMFindDataRef("sim/flightmodel/engine/ENGN_running");

	mDPlaneAnimOvverides.emplace("LAT", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("LON", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("ELEV", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("PITCH", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("ROLL", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("HDG", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("GEAR", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("FLAP", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("SPDBRK", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("WNGSWP", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("THRUST", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("LTAXI", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("LLAND", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("LBCN", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("LSTRB", std::make_pair(false, 0.0));
	mDPlaneAnimOvverides.emplace("LNAV", std::make_pair(false, 0.0));
}
	...
	if(p2penabled) {
		...
		switch(_stunclient.RunTest()) {
			...
			case Stun::StunIndependentFilter:
				...
				xivap.addText(colWhite, "STUN determined active p2p (" + type + ")", true, false);
				_p2pmode = 3;
				break;
			...
			case Stun::StunDependentMapping:
				...
				xivap.addText(colWhite, "STUN determined passive p2p (" + type + ")", true, false);
				_p2pmode = 2;
				break;
			...
			case Stun::StunFirewall:
				...
				xivap.addText(colWhite, "STUN determined NO p2p (" + type + ")", true, false);
				_p2pmode = 0;
				break;

			case Stun::StunError:
			default:
				xivap.addText(colWhite, "STUN ERROR", true, false);
				_p2pmode = 0;
				break;
		}
	}
...
bool MultiplayerEngine::enable()
{
	...
	if(!_initialized) init(xivap.p2p_enabled(), xivap.p2p_max_sendbps(), xivap.p2p_max_recvbps(), xivap.p2p_forced_port(), xivap.default_icao());
...
void Multiplayer::MultiplayerEngine::addDebugModel(string aircraft, string airline, string livery, double elevOffset, double latOffset, double lonOffset) {
	// #drawtest
	removeDebugModel();
					...
					void * inRefcon)
{
	...
	if (inPlane == gTestPlaneId) {
		...
		callsign = planeTest.mtl;
		planeTest.setLightPattern(gTestPlaneICAO);

		// plane position
		planeTest.position.pos.lat = xivap.GetLat() + gTestPlaneLatOffset;
		auto &pair = mDPlaneAnimOvverides["LAT"];
		if (pair.first) {
			planeTest.position.pos.lat = pair.second;
		}
		planeTest.position.pos.lon = xivap.GetLon() + gTestPlaneLonOffset;
		pair = mDPlaneAnimOvverides["LON"];
		if (pair.first) {
			planeTest.position.pos.lon = pair.second;
		}
		planeTest.position.pos.elevation = xivap.elevationft() + gTestPlaneElevOffset;
		pair = mDPlaneAnimOvverides["ELEV"];
		if (pair.first) {
			planeTest.position.pos.elevation = pair.second;
		}
		planeTest.position.pos.heading = xivap.GetHeading();
		pair = mDPlaneAnimOvverides["HDG"];
		if (pair.first) {
			planeTest.position.pos.heading = pair.second;
		}
		planeTest.position.pos.roll = xivap.GetBank();
		pair = mDPlaneAnimOvverides["ROLL"];
		if (pair.first) {
			planeTest.position.pos.roll = pair.second;
		}
		planeTest.position.pos.pitch = xivap.GetPitch();
		pair = mDPlaneAnimOvverides["PITCH"];
		if (pair.first) {
			planeTest.position.pos.pitch = pair.second;
		}
		planeTest.onground = xivap.onGround();
		// denis maslov 22/08/2017
		// get real ground elevation and set it as multiplayer plane elev if it's on the ground and the clamping function is enabled.
		correctElevOfPosition(planeTest.onground, xivap.Clamping(), planeTest.position.pos);

		// plane params
		planeTest.surfaces.gearPosition = XPLMGetDatai(mDPlaneGearDeploy);
		pair = mDPlaneAnimOvverides["GEAR"];
		if (pair.first) {
			planeTest.surfaces.gearPosition = pair.second;
		}
		planeTest.surfaces.flapRatio = XPLMGetDataf(mDPlaneFlapsDeploy);
		pair = mDPlaneAnimOvverides["FLAP"];
		if (pair.first) {
			planeTest.surfaces.flapRatio = pair.second;
		}
		planeTest.surfaces.slatRatio = XPLMGetDataf(mDPlaneFlapsDeploy) >= 0.1 ? 1.0f : 0.0f;
		planeTest.surfaces.speedBrakeRatio = XPLMGetDataf(mDPlaneSpeedBrakes);
		planeTest.surfaces.spoilerRatio = XPLMGetDataf(mDPlaneSpeedBrakes);
		pair = mDPlaneAnimOvverides["SPDBRK"];
		if (pair.first) {
			planeTest.surfaces.speedBrakeRatio = pair.second;
			planeTest.surfaces.spoilerRatio = pair.second;
		}

		int intArray[8];
		float floatArray[8];
		float average = 0;
		int isEnginesRunning = false;
		// num engines
		int numEngines = XPLMGetDatai(mDPLaneNumEngines);
		// is running
		XPLMGetDatavi(mDPlaneIsEngineRunning, intArray, 0, 8);
		for (int i = 0; i < numEngines; ++i) {
			if (intArray[i] != 0) {
				isEnginesRunning = true;
				break;
			}
		}
		// if is running
		if (isEnginesRunning) {
			// reverse
			XPLMGetDatavi(mDPlaneEngineMode, intArray, 0, 8);
			bool isreverseActive = false;
			if (intArray[0] == 3) isreverseActive = true;
			// thrust
			XPLMGetDatavf(mDPlaneEngineThrust, floatArray, 0, 8);
			for (int i = 0; i < numEngines; ++i) {
				average += floatArray[i];
			}
			average = average / numEngines;
			average = std::max(average, 0.1f);
			planeTest.surfaces.thrust = isreverseActive ? -1.0 : average;
		}
		else {
			planeTest.surfaces.thrust = 0.0f;
		}
		pair = mDPlaneAnimOvverides["THRUST"];
		if (pair.first) {
			planeTest.surfaces.thrust = pair.second;
		}

		// wing flex
		planeTest.surfaces.wingSweep = planeTest.onground ? 0.8f : 0.0f;
		pair = mDPlaneAnimOvverides["WNGSWP"];
		if (pair.first) {
			planeTest.surfaces.wingSweep = pair.second;
		}

		// we do not have this data in fsd network
		planeTest.surfaces.yokeHeading = 0;
		planeTest.surfaces.yokePitch = 0;
		planeTest.surfaces.yokeRoll = 0;

		// lights
		planeTest.surfaces.lights.taxiLights = 0;
		planeTest.surfaces.lights.landLights = 0;
		planeTest.surfaces.lights.navLights = 0;
		planeTest.surfaces.lights.strbLights = 0;
		planeTest.surfaces.lights.bcnLights = 0;
		if (XPLMGetDatai(mDPlaneTaxiRef)) planeTest.surfaces.lights.taxiLights = 1;
		if (XPLMGetDatai(mDPlaneBeaconRef)) planeTest.surfaces.lights.bcnLights = 1;
		if (XPLMGetDatai(mDPlaneStrobeRef)) planeTest.surfaces.lights.strbLights = 1;
		if (XPLMGetDatai(mDPlaneNavRef)) planeTest.surfaces.lights.navLights = 1;
		if (XPLMGetDatai(mDPlaneLandingRef)) planeTest.surfaces.lights.landLights = 1;
		pair = mDPlaneAnimOvverides["LTAXI"];
		if (pair.first) {
			planeTest.surfaces.lights.taxiLights = pair.second;
		}
		pair = mDPlaneAnimOvverides["LLAND"];
		if (pair.first) {
			planeTest.surfaces.lights.landLights = pair.second;
		}
		pair = mDPlaneAnimOvverides["LBCN"];
		if (pair.first) {
			planeTest.surfaces.lights.bcnLights = pair.second;
		}
		pair = mDPlaneAnimOvverides["LSTRB"];
		if (pair.first) {
			planeTest.surfaces.lights.strbLights = pair.second;
		}
		pair = mDPlaneAnimOvverides["LNAV"];
		if (pair.first) {
			planeTest.surfaces.lights.navLights = pair.second;
		}
	...
	switch(packet.type) {
		case _FSD_PILOTPOS_:
			...
			if(position != NULL) {
				...
				//denis maslov 22/08/2017
				// get real ground elevation and set it as multiplayer plane elev if it's on the ground and the clamping function is enabled.
				correctElevOfPosition(onground, xivap.Clamping(), *position);
			}
			break;
...
//denis maslov 22/08/2017
void MultiplayerEngine::correctElevOfPosition(bool inIsOnGround, bool inIsClampingIsOn, XPMPPlanePosition_t &inOutPlanePos) {
	if (inIsOnGround && inIsClampingIsOn) {
		double x, y, z = 0;
		XPLMProbeInfo_t probeInfo;
		probeInfo.structSize = sizeof(XPLMProbeInfo_t);
		XPLMWorldToLocal(inOutPlanePos.lat, inOutPlanePos.lon, inOutPlanePos.elevation * 0.3048/*feet to meters*/, &x, &y, &z);
		XPLMProbeResult probeRes = XPLMProbeTerrainXYZ(mTerrainProbe, x, y, z, &probeInfo);
		if (probeRes == xplm_ProbeHitTerrain) {
			double lat, lon, correctElev;
			XPLMLocalToWorld(probeInfo.locationX, probeInfo.locationY, probeInfo.locationZ, &lat, &lon, &correctElev);
			correctElev = correctElev * 3.28084/* meters to feet*/;
			inOutPlanePos.elevation = correctElev;
		}
	}
}
...
bool Multiplayer::MultiplayerEngine::createMTL(string aircraft, string airline, string livery, string &outMtl) {
	if (length(aircraft) < 2 || length(aircraft) > 4) return false;
	if (!isempty(airline) && length(airline) != 3) return false;
	...
	if (isempty(airline)) {
		...
		os << std::left << std::setw(4) << std::setfill(' ') << aircraft.stl_str();
		outMtl = os.str();
		os.str("");
		os << std::right << std::setw(3) << std::setfill('@') << airline.stl_str();
		outMtl += os.str();
		if (!isempty(livery)) {
			outMtl += livery;
		}
...
bool Multiplayer::MultiplayerEngine::createMTL(string mtlWithSlashes, string &outMtl) {
	std::vector<std::string> splitMtl;
	splitMtl = xmp::explode(mtlWithSlashes, "/");
	if (splitMtl.empty()) {
		return false;
	}
	string icao = splitMtl[0];
	string airline;
	string livery;
	if (splitMtl.size() > 1) {
		airline = splitMtl[1];
	...
	return createMTL(icao, airline, livery, outMtl);
}
...
void MultiplayerEngine::HandlePlaneParams(MultiplayerPilot *pilot, FSD::PlaneParams params)
{
	...
	pilot->surfaces.lights.taxiLights = 0;
	...
	if(params.taxiLight) pilot->surfaces.lights.taxiLights = 1;
	...
	pilot->surfaces.flapRatio			= std::max(-1.0f, std::min(1.0f, params.flapsRatio / 7.f));
	pilot->surfaces.speedBrakeRatio	    = std::max(-1.0f, std::min(1.0f, params.speedbrakeRatio / 7.f));
	pilot->surfaces.spoilerRatio		= std::max(-1.0f, std::min(1.0f, params.speedbrakeRatio / 7.f));

	pilot->surfaces.thrust              = std::max(-1.0f, std::min(1.0f, params.thrustRatio / 7.f));
	if (params.engine1Running || params.engine2Running || params.engine3Running || params.engine4Running) {
		pilot->surfaces.thrust = std::max(pilot->surfaces.thrust, 0.1f);
		if (params.thrustReversers) {
			pilot->surfaces.thrust = -1.0f;
		}
	}
	else {
		pilot->surfaces.thrust = 0.0f;
	}

	// we do not have this data in fsd network
	pilot->surfaces.yokeHeading = 0;
	pilot->surfaces.yokePitch = 0;
	pilot->surfaces.yokeRoll = 0;
...
void MultiplayerEngine::ProcessPPOS1Pos(MultiplayerPilot *pilot, const char *buffer, int buflen)
{
	...
	*/
	...
	pos->onground = pilot->onground;
	// den_rain: I've forgotten to add this into processing p2p packets.
	correctElevOfPosition(pos->onground, xivap.Clamping(), pos->pos);
