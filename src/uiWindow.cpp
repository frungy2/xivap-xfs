...
UiWindow::UiWindow():
	mMaxCmdLineHistorySize(512),
	mCurrentCmdLineHistoryPos(-1),
	mCursorPos(0)
{
	mDownX = mDownY = 0;
	dragging = false;
...
void UiWindow::pluginStart()
{
	...
	if (xivap.saveCmdHistoryBetweenSessions()) {
		std::string fileName = getXivapRessourcesDir().stl_str() + CMD_LINE_HISTORY_FILE;
		...
		if (file.is_open()) {
			while (file.good()) {
				...
				mCmdLineHistory.push_back(line);
				++lineCounter;
				if (lineCounter > mMaxCmdLineHistorySize) {
					break;
				}
			}
			mCurrentCmdLineHistoryPos = static_cast<int>(mCmdLineHistory.size());
			file.close();
		}
		else {
			XPLMDebugString(std::string("X-IvAp Warning: Can't open the cmd line history file: " + fileName + "\n").c_str());
		}
	}
}
...
void UiWindow::create()
{
	...
	ConfigFile config;
	pt::string str = getXivapRessourcesDir() + CONFIG_FILE;
	config.load(str);
		...
		void *inRefcon, int losingFocus)
{
	...
	if((inFlags & (xplm_OptionAltFlag)) != 0) {
		// Alt key (Alt doesnt work?) is down
		// This is a shortcut of some kind -> get rid of the keyboard focus
		dumpKeyboardFocus();
		return;
	}
	if ((inFlags & (xplm_ControlFlag)) != 0 && (inKey < 28 || inKey > 31)) {
		// Ctrl is down
		// This is a shortcut of some kind -> get rid of the keyboard focus
		// except our arrow keys
		dumpKeyboardFocus();
		return;
	}
	...
	if(((inFlags & xplm_DownFlag) != 0) // key down
		...
		switch(inVirtualKey) {
			...
			case XPLM_VK_RETURN:
				keyEnter(); break;
			case XPLM_VK_BACK: keyBackspace(); break;
			case XPLM_VK_DELETE: keyDelete(); break;
			case XPLM_VK_HOME: keyHome(); break;
			case XPLM_VK_END: keyEnd(); break;
			case XPLM_KEY_UP: 
			case XPLM_VK_UP:
				keyUpArrow(inFlags); break;
			case XPLM_KEY_DOWN: 
			case XPLM_VK_DOWN:
				keyDownArrow(inFlags); break;
			case XPLM_KEY_LEFT: 
			case XPLM_VK_LEFT:
				keyLeftArrow(inFlags); break;
			case XPLM_KEY_RIGHT: 
			case XPLM_VK_RIGHT:
				keyRightArrow(inFlags); break;
			case XPLM_VK_TAB:
			case XPLM_VK_CLEAR:
			case XPLM_VK_ESCAPE:
			case XPLM_VK_PRIOR:
			...
			case XPLM_VK_SELECT:
			case XPLM_VK_PRINT:
			case XPLM_VK_EXECUTE:
			case XPLM_VK_SNAPSHOT:
			case XPLM_VK_INSERT:
			case XPLM_VK_HELP:
				break;
			default: 
				if (inVirtualKey < XPLM_VK_F1 || inVirtualKey > XPLM_VK_F24) {
					handleKey(inKey);
				}
				break;
		}
		...
		mIsCursorVisible = true;	// show cursor after keystroke
		mCursorTime = _lastKeyPress;
...
void UiWindow::addText(UI_Alignment align, int line, float *color, const pt::string& text, bool big, bool infms)
{
	UIWriteJob job;
	job.text = text;
	job.color = color;
...
void UiWindow::_appendText(float *color, const pt::string& line)
{
...
void UiWindow::addMessage(float *color, const pt::string& text, bool logToConsole, bool logToLogfile)
{
		...
		case SCREEN_ACARS:
			switch(button) {
				...
				case LSK2:
					...
					break;
				case LSK3:
					xivap.fsd.sendWxRequest(FSD::WX_TAF, mEditLine);
					mEditLine = "";
					screen = SCREEN_MAIN;
					break;
				case LSK4:
					xivap.fsd.sendWxRequest(FSD::WX_SHORTTAF, mEditLine);
					mEditLine = "";
					screen = SCREEN_MAIN;
					break;
				case LSK5:
					xivap.fsd.sendWxRequest(FSD::WX_METAR, mEditLine);
					mEditLine = "";
					screen = SCREEN_MAIN;
					break;
				case RSK1:
					...
					break;
				case RSK2:
					xivap.fsd.sendInfoRequest(mEditLine, _FSD_INFOREQ_ATIS_);
					mEditLine = "";
					screen = SCREEN_MAIN;
					break;
				case RSK5:
...
#endif
				case LSK6:
					xivap.setClamping(!xivap.Clamping());
					break;
				case RSK5:
					screen = SCREEN_SYSTEM;
					break;
...
void UiWindow::prepScreen()
{
	...
	switch(screen) {
		...
		default:
			...
			addText(AL_left, 3, colGreen, pt::string(SOFTWARE_NAME) + " " + SOFTWARE_VERSION
#ifdef IVAO
				+ " IVAO"
#endif
#ifdef XFS
			...
			#ifdef XFS 
			addText(AL_left, 7, colGreen, XFLIGHTSERVER, true);
			#endif
		...
		case SCREEN_ACARS: {
				...
				pt::string line = "-"; while(length(line) < ui_columns) line += "-";
				addText(AL_left, 10, colDarkGreen, line, true);
			}
			break;
...
*/
		...
		case SCREEN_PREFS:
			...
			addText(AL_left, 1, colWhite, pt::string("<WX ") + (xivap.usingWeather() ? "ON" : "OFF"), true);
			addText(AL_left, 2, colDarkGreen, "MULTIPLAYER", false);
			addText(AL_left, 3, colWhite, pt::string("<MP ") + (xivap.usingMultiplayer() ? "ON" : "OFF"), true);
			addText(AL_left, 5, colWhite,pt::string("<Tagging ") + (xivap.usingLabels() ? "ON" : "OFF"), true);
			addText(AL_left, 7, colWhite,pt::string("<Private MSG alert ") + (xivap.usingPMSG() ? "ON" : "OFF"), true);
			addText(AL_left, 8, colDarkGreen, "TeamSpeak", false);
#ifdef HAVE_TEAMSPEAK
			addText(AL_left, 9, colWhite, pt::string("<VOICE ") + (xivap.usingVoice() ? "ON" : "OFF"), true);
#else
			addText(AL_left, 9, colGreen, "INOP", true);
#endif
			addText(AL_left, 11, colWhite, pt::string("<ClampPlanes ") + (xivap.Clamping() ? "ON" : "OFF"), true);
	...
	if(onEditPage()) {
		...
		if(!_keyboardfocus)	{
			...
			if(elapsed >= mCursorTime + CURSOR_INTERVAL) {
...
bool UiWindow::selectStation(int sk)
{
	...
	pt::string s = copy(_atcFreqs[sk], 0, 3) + copy(_atcFreqs[sk], 4, 3);

	// convert "123455" to 123455
	int freq = static_cast<int>(stringtoi(s));
    pt::string name = _atcNames[sk];
	// and tune da shit
...
void UiWindow::keyEnter()
{
	...
	if (mCmdLineHistory.size() > mMaxCmdLineHistorySize) {
		mCmdLineHistory.erase(mCmdLineHistory.begin());
	}
	for (size_t i = mCmdLineHistory.size(); i > 0; --i) {
		size_t actualIdx = i - 1;
		if (mCmdLineHistory[actualIdx] == mEditLine.stl_str()) {
			mCmdLineHistory.erase(mCmdLineHistory.begin() + actualIdx);
			break;
		}
	}
	mCmdLineHistory.push_back(mEditLine.stl_str());
	mCurrentCmdLineHistoryPos = static_cast<int>(mCmdLineHistory.size());

	xivap.processLine(mEditLine);
	mEditLine = "";
	mCursorPos = 0;
	textLine = 0;//scrolls to down
}

void UiWindow::keyBackspace() {
	if(!onEditPage()) return;
	if (length(mEditLine) > 0) {
		//del(mEditLine, length(mEditLine) - 1, 1);
		del(mEditLine, --mCursorPos, 1);
	}
}

void UiWindow::keyDelete() {
	if (!onEditPage()) return;
	if (length(mEditLine) > 0) {
		del(mEditLine, mCursorPos, 1);
	}
...
void UiWindow::keyHome() {
	if (!onEditPage()) return;
	mCursorPos = 0;
}

void UiWindow::keyEnd() {
	if (!onEditPage()) return;
	mCursorPos = length(mEditLine);
}

void UiWindow::keyUpArrow( XPLMKeyFlags inFlags ) {
	if (!onEditPage()) return;
	if ((!xivap.modernKeyMapForCmdHistory() && (inFlags & (xplm_ControlFlag)) != 0)
		|| (xivap.modernKeyMapForCmdHistory() && (inFlags & (xplm_ControlFlag)) == 0)) {

		if (mCmdLineHistory.empty()) {
			return;
		}
		// init history walking
		if (mCurrentCmdLineHistoryPos >= static_cast<int>(mCmdLineHistory.size())) {// init
			mCurrentCmdLineHistoryPos = static_cast<int>(mCmdLineHistory.size()) - 1;
			mSavedEditLine = mEditLine;
		}
		else if (mCurrentCmdLineHistoryPos > -1){// decrement
			--mCurrentCmdLineHistoryPos;
		}
		// setup line from history
		if (mCurrentCmdLineHistoryPos >= 0 && mCurrentCmdLineHistoryPos < static_cast<int>(mCmdLineHistory.size())) {			
			mEditLine = mCmdLineHistory[mCurrentCmdLineHistoryPos];
			mCursorPos = length(mEditLine);
		}
		return;
	}
...
void UiWindow::keyDownArrow( XPLMKeyFlags inFlags ) {
	if (!onEditPage()) return;
	if ((!xivap.modernKeyMapForCmdHistory() && (inFlags & (xplm_ControlFlag)) != 0)
		|| (xivap.modernKeyMapForCmdHistory() && (inFlags & (xplm_ControlFlag)) == 0)) {
		...
		// reinit history walking
		if (mCurrentCmdLineHistoryPos < 0) {
			mCurrentCmdLineHistoryPos = 1;
		}
		else if (mCurrentCmdLineHistoryPos < static_cast<int>(mCmdLineHistory.size())) {//increment
			++mCurrentCmdLineHistoryPos;
		}		
		if (mCurrentCmdLineHistoryPos >= 0 && mCurrentCmdLineHistoryPos < static_cast<int>(mCmdLineHistory.size())) {
			mEditLine = mCmdLineHistory[mCurrentCmdLineHistoryPos];
		...
		else if (mCurrentCmdLineHistoryPos == static_cast<int>(mCmdLineHistory.size())){
			mEditLine = mSavedEditLine;
			mCursorPos = length(mEditLine);
			clear(mSavedEditLine);
			++mCurrentCmdLineHistoryPos;
		}
		return;
	}
...
void UiWindow::keyLeftArrow( XPLMKeyFlags inFlag ) {
	if(!onEditPage()) return;
	if (mCursorPos > 0) {
		--mCursorPos;
	}
...
void UiWindow::keyRightArrow( XPLMKeyFlags inFlags ) {
	if(!onEditPage()) return;
	if (mCursorPos < length(mEditLine)) {
		++mCursorPos;
	}
}
...
void UiWindow::handleKey(int key) {
	if(!onEditPage()) return;
	if (key == '"') key = '\'';
	if (length(mEditLine) < 300) {
		//mEditLine += key;
		ins(key, mEditLine, mCursorPos);
		++mCursorPos;
	}
}
